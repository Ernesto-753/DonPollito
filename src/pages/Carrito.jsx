...
export default function Carrito() {
  ...
  const articulos = Object.entries(carrito)
    .map(([id, cantidad]) => ({ id, cantidad, producto: obtenerProducto(id) }))
    .filter((item) => item.producto);
  const hayProductos = articulos.length > 0;
      ...
      <div className="grid lg:grid-cols-3 gap-10 items-start max-w-5xl">
        <Card className="lg:col-span-2 divide-y divide-black/10 shadow-none border border-black/10">
          {!hayProductos ? (
            ...
            articulos.map(({ id, cantidad, producto }) => (
